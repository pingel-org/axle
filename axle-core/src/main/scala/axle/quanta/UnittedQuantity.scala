...
object UnittedQuantity4 {
  ...
  implicit def eqqqn[Q <: Quantum4[N], N: Eq]: Eq[UnittedQuantity4[Q, N]] =
    new Eq[UnittedQuantity4[Q, N]] {
      def eqv(x: UnittedQuantity4[Q, N], y: UnittedQuantity4[Q, N]): Boolean =
        (x.magnitude === y.magnitude) && (x.unit == y.unit)
    }

  implicit def orderUQ[Q <: Quantum4[N], N: MultiplicativeMonoid: Order, DG[_, _]: DirectedGraph](implicit cg: DG[UnitOfMeasurement4[Q, N], N => N]) =
    new Order[UnittedQuantity4[Q, N]] {

      val orderN = implicitly[Order[N]]

      def compare(x: UnittedQuantity4[Q, N], y: UnittedQuantity4[Q, N]): Int =
        orderN.compare((x.in(y.unit)).magnitude, y.magnitude)
    }

}

case class UnittedQuantity4[Q <: Quantum4[N], N](magnitude: N, unit: UnitOfMeasurement4[Q, N]) {

  private[this] def vertex[DG[_, _]: DirectedGraph](
    cg: DG[UnitOfMeasurement4[Q, N], N => N],
    query: UnitOfMeasurement4[Q, N])(
      implicit ev: Eq[N]): Vertex[UnitOfMeasurement4[Q, N]] =
    axle.syntax.directedgraph.directedGraphOps(cg).findVertex(_.payload.name === query.name).get

  def in[DG[_, _]: DirectedGraph](
    newUnit: UnitOfMeasurement4[Q, N])(
      implicit cg: DG[UnitOfMeasurement4[Q, N], N => N], ev: MultiplicativeMonoid[N], ev2: Eq[N]): UnittedQuantity4[Q, N] =
    directedGraphOps(cg).shortestPath(vertex(cg, newUnit), vertex(cg, unit))
      .map(
        _.map(_.payload).foldLeft(ev.one)((n, convert) => convert(n)))
      .map(n => UnittedQuantity4((magnitude * n), newUnit))
      .getOrElse(throw new Exception("no conversion path from " + unit + " to " + newUnit))
