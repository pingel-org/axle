...
class ABE extends Specification {
  ...
  val bn = BayesianNetwork[Boolean](
    ...
    List(BayesianNetworkNode[Boolean](B,
      Factor(Vector(B), Map(
        Vector(B is true) -> 0.001,
        Vector(B is false) -> 0.999))),
      BayesianNetworkNode(E,
        Factor(Vector(E), Map(
          Vector(E is true) -> 0.002,
          Vector(E is false) -> 0.998))),
      BayesianNetworkNode(A,
        Factor(Vector(B, E, A), Map(
          Vector(B is false, E is false, A is true) -> 0.001,
          Vector(B is false, E is false, A is false) -> 0.999,
          Vector(B is true, E is false, A is true) -> 0.94,
          Vector(B is true, E is false, A is false) -> 0.06,
          Vector(B is false, E is true, A is true) -> 0.29,
          Vector(B is false, E is true, A is false) -> 0.71,
          Vector(B is true, E is true, A is true) -> 0.95,
          Vector(B is true, E is true, A is false) -> 0.05))),
      BayesianNetworkNode(J,
        Factor(Vector(A, J), Map(
          Vector(A is true, J is true) -> 0.9,
          Vector(A is true, J is false) -> 0.1,
          Vector(A is false, J is true) -> 0.05,
          Vector(A is false, J is false) -> 0.95))),
      BayesianNetworkNode(M,
        Factor(Vector(A, M), Map(
          Vector(A is true, M is true) -> 0.7,
          Vector(A is true, M is false) -> 0.3,
          Vector(A is false, M is true) -> 0.01,
          Vector(A is false, M is false) -> 0.99)))),
