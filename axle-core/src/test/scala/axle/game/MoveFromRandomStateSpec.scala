...
class MoveFromRandomStateSpec extends FunSuite with Matchers {
  ...
  implicit val eqS: Eq[TestGameState] = Eq[TestGameState]
  ...
  val movesMap: Map[TestGameState, Map[TestGameMove, (TestGameState, Rational)]] = Map(
    Sa -> Map((Maa, (Sa, Rational(1, 2))), (Mab, (Sb, Rational(1, 4))), (Mac, (Sc, Rational(1, 4)))),
    Sb -> Map((Mba, (Sa, Rational(0))),    (Mbb, (Sb, Rational(1))),    (Mbc, (Sc, Rational(0)))),
    Sc -> Map((Mca, (Sa, Rational(0))),    (Mcb, (Sb, Rational(0))),    (Mcc, (Sc, Rational(1))))
  )
    ...
    new Game[TestGame,TestGameState,TestGameOutcome,TestGameMove,TestGameState,TestGameMove,Rational,ConditionalProbabilityTable] {
      ...
      def strategyFor(game: TestGame, player: Player): (TestGame, TestGameState) => ConditionalProbabilityTable[TestGameMove, Rational] =
        (game: TestGame, state: TestGameState) => {
          ...
          pm.construct(Variable("S"), mm.keys, mm.mapValues(_._2))
        }
  ...
  (1 to 20) foreach { i =>
    val (stateMoveOpt, newStateModel) = moveFromRandomState(game, currentStateModel, rng)
    println(s"$stateMoveOpt -> $newStateModel")
  }
  // makeSomeAssertions
  ...
  val SaMacModelMap = Map(Sa -> Rational(1, 4), Sb -> Rational(2, 3), Sc -> Rational(1, 12))
  val SaMacStateModel = pm.construct(Variable("S"), SaMacModelMap.keys, SaMacModelMap)

  // Randomly select Sa then Mab
  //   Sa: (1/3, -1/4) => 1/3 + (1/3 . -1/4) = 1/4
  //   Sb: (2/3,  1/4) => 2/3 + (1/3 . -1/4) = 3/4

  val SaMabModelMap = Map(Sa -> Rational(1, 4), Sb -> Rational(3, 4))
  val SaMabStateModel = pm.construct(Variable("S"), SaMabModelMap.keys, SaMabModelMap)
