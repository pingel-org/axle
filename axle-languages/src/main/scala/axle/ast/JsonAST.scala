...
object JsonAST {
  ...
  def obj2ast(obj: Any, parentLineNo: Int): AstNode = {

//    println("Case for object: " + obj)
//    if (obj != null) {
//      println(".getClass.getName = " + obj.getClass.getName)
//    }
    ...
    obj match {
      ...
      case x: Map[_, _] => {
        ...
        val lineNo = m.get("_lineno").asInstanceOf[Option[String]].map(_.toInt).getOrElse(parentLineNo)
      ...
      case x: collection.convert.Wrappers$JMapWrapper => {
        val m = x.toMap.asInstanceOf[Map[String, Any]]
        val lineNo = m.get("_lineno").asInstanceOf[Option[String]].map(_.toInt).getOrElse(parentLineNo)
        val metaMap = m.keySet.filter(field => (field != "type") && (field != "_lineno")).toList
          .map(field => field -> obj2ast(m(field), lineNo))
          .toMap
