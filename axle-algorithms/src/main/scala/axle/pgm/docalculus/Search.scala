...
object Search {
  ...
  def expand[T: Eq, N: Field, DG](model: CausalModel[T, N, DG], quantity: CausalityProbability[T, N], namer: VariableNamer[T, N]): Some[List[Form]] =
  ...
  def reduce[T: Eq, N: Field, DG](model: CausalModel[T, N, DG], quantity: CausalityProbability[T, N], namer: VariableNamer[T, N], depth: Int, maxDepth: Int): Option[List[Form]] =
    if (depth <= maxDepth) {
