...
class GeoLengthSpaceSpec extends Specification {
  ...
  "geo metric space" should {
    "measure distance from San Francisco to New York" in {
      ...
      degreesDistance must be equalTo 18.564484707158623
  ...
  "geo length space" should {
    "calculate the way-point 40% from LAX to JFK correctly" in {
      ...
      (waypoint.latitude in °).magnitude must be equalTo 38.64825118848812
      (waypoint.longitude in °).magnitude must be equalTo 101.7338280099326
    }
  }
