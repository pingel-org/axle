...
class ScalaFigures extends Specification {
  ...
  def figure6_4() = {
    ...
    val av = bn += BayesianNetworkNode(A, Factor(Vector(A), Map(
      List(A eq true) -> 0.6,
      List(A eq false) -> 0.4
    )))

    // B | A
    val bv = bn += BayesianNetworkNode(B, Factor(Vector(B), Map(
    ...
    // C | B
    val cv = bn += BayesianNetworkNode(C, Factor(Vector(C), Map(
