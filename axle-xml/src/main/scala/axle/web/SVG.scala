...
object SVG {
  ...
  implicit def svgPlot[S, X, Y, D]: SVG[Plot[S, X, Y, D]] = new SVG[Plot[S, X, Y, D]] {
    ...
    def svg(plot: Plot[S, X, Y, D]): NodeSeq = {
      ...
      val view = PlotView(plot, plot.dataFn())
  ...
  implicit def svgBarChart[C, Y, D, H]: SVG[BarChart[C, Y, D, H]] =
    new SVG[BarChart[C, Y, D, H]] {
      ...
      def svg(chart: BarChart[C, Y, D, H]): NodeSeq = {
        ...
        val view = BarChartView(chart, chart.dataFn())
  ...
  implicit def svgBarChartGrouped[G, S, Y, D, H]: SVG[BarChartGrouped[G, S, Y, D, H]] =
    new SVG[BarChartGrouped[G, S, Y, D, H]] {
      ...
      def svg(chart: BarChartGrouped[G, S, Y, D, H]): NodeSeq = {
        ...
        val view = BarChartGroupedView(chart, chart.dataFn())
  ...
  implicit def svgBayesianNetworkVisualization[T: Eq, N: Field: Eq, DG](
    ...
    new SVG[BayesianNetworkVisualization[T, N, DG]] {
      def svg(vis: BayesianNetworkVisualization[T, N, DG]): NodeSeq = {
        import vis._
        val subVis = DirectedGraphVisualization[DG, BayesianNetworkNode[T, N], axle.pgm.Edge](vis.bn.graph, width, height, border)
        svgDGVis.svg(subVis)
      }
    }
  }
